...
function ShoppingList() {
  ...
  const handleKeyDown = async (e: KeyboardEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!updated.ok) {
        throw new Error(`HTTP error! status: ${updated.status}`);
      }
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            {products.map((item, index) => (
              <div
                key={item.id}
                className="flex items-center justify-between p-3 rounded-lg border bg-card bg-white/70 hover:bg-muted/50 transition-colors"
              >
                <div className="flex items-center space-x-3 bg-white/70">
                  ...
                  />
                  <span
                    className={`flex-1 ${item.checked ? "line-through text-muted-foreground" : "text-foreground"}`}
                  >
                    {item.ingredients}
                  </span>
                </div>
